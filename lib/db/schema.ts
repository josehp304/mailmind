...
// Daily email summaries generated by AI
export const dailyEmailSummaries = pgTable('daily_email_summaries', {
  id: uuid('id').defaultRandom().primaryKey(),
  userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
  date: timestamp('date').notNull(), // Date this summary is for
  lastEmailId: varchar('last_email_id', { length: 255 }).notNull(), // Last processed Gmail ID
  summary: text('summary').notNull(), // Full AI summary
  snippet: text('snippet').notNull(), // Short preview text
...
export const dailyEmailSummariesRelations = relations(dailyEmailSummaries, ({ one }) => ({
  user: one(users, { fields: [dailyEmailSummaries.userId], references: [users.id] }),
}));
